...
- **跑测试**:`cd sidecar && npm test`(当前 65 个,11 文件)+`npm run test:ui`(当前 18 项,desktop/narrow 各执行 9 个场景)。新增逻辑配单测。MATLAB 侧用 `matlab -batch` + `checkcode`(纯静态语法)/`meta.class.fromName`(类加载)/直接调静态方法(如工程索引逻辑)无界面自检,临时 `.m` 跑完删掉。**适配器并发改动**用 fake child(EventEmitter,覆盖 `spawnChild`)测 busy 保护 / per-child 守卫 / reset 不回写,不真 spawn CLI。
