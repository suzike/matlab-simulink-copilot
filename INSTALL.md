# MATLAB Copilot v0.13.0 — 安装与使用指南
...
`setupMATLABCopilot("repair")` 会检查工具箱入口是否位于 MATLAB 路径；常规 pathdef 无法持久化时，它只在用户 `startup.m` 中维护带起止标记的 MATLAB-Copilot 路径块。可随时运行 `setupMATLABCopilot("status")` 查看状态。
...
1. 下载 v0.13.0 的 `MATLAB-Copilot.mltbx`。
2. 关闭现有 Copilot 面板后安装新包。
3. 运行 `setupMATLABCopilot("repair")` 和 `copilot_doctor()`，确认入口路径、Node、后端 CLI、MATLAB MCP、会话共享、sidecar 文件和端口检查通过。
4. 发布维护者可运行 `release_acceptance('MATLAB-Copilot.mltbx')`，从最终安装包执行类加载、静态检查和 Echo TCP 冒烟。
5. 检查沙漏按钮右侧的快捷功能三态：隐藏、单行、多行；单行模式应能用鼠标滚轮浏览且不显示滚动条。
6. 启动模型变更记录器，填写任务和需求 ID，保存一次模型后确认时间线出现文件级及块级语义变化。
7. 导出任务证据包，确认包含 `change-report.md`、`manifest.json`、`changes.jsonl`、`evidence-index.json`、`traceability.json` 和 `evidence-integrity.json`，并可通过完整性检查重新载入。
8. 在 capabilities 未就绪或模型列表为空时，后端名称后不应出现孤立下拉箭头。
...
先在 MATLAB 中运行 `setupMATLABCopilot("uninstall")`，删除本工具管理的持久路径块；然后：
