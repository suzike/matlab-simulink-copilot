...
- 仅当修改前模型已保存、无未保存改动且基线可编译时启用自动回退；验证失败恢复原 `.slx` 并重新加载，脏模型明确转人工恢复，绝不覆盖原有未保存内容。
- 无完整 block path 的 MCP 编辑也建立模型级事务；事务输入在落盘前递归脱敏 token、secret、password、Authorization、cookie 和 API key 字段。
- UI 新增可信变更事务卡，展示编译、规范、回退状态和证据文件位置。
- 新增工程级 `ProjectChangeRecorder`：显式启动后扫描当前 MATLAB 工程并建立基线，通过 Windows 递归文件监视与逐文件防抖记录保存事件；证据目录独立于工程，保留前后快照、哈希、时间、来源和文本变化摘要。
- `ChangeTransaction` 完成后把未保存的 AI 模型语义变化直接写入工程记录器；人工/脚本修改在保存时进入同一时间线，避免注入或改写用户模型回调。
- 保存的 `.slx/.mdl` 修改由 `ModelFileDiff` 将前后证据复制为唯一临时模型名后隔离加载，输出参数变化、新增/删除块和块数量；分析结果按原 `id + sequence` 回写，不重复计数，失败不影响原始证据。
